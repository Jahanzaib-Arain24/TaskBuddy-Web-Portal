							...
							<div class="GridLex-col-9_sm-8_xs-12">
								...
								<div class="admin-content-wrapper">
									...
									<div class="admin-section-title">
										...
										<h2 class="admin-title-heading"><i class="fa fa-users"></i> References & Referees</h2>
										<p class="admin-title-sub">Provide contact references from past clients, supervisors, or employers</p>
                                    ...
                                    $page1_int = (int)$page1;
                                    $stmt = $conn->prepare("SELECT * FROM tbl_referees WHERE member_no = :myid ORDER BY id LIMIT $page1_int,5");
								...
								try {
                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                $stmt = $conn->prepare("SELECT COUNT(*) as total FROM tbl_referees WHERE member_no = :myid");
                                $stmt->bindParam(':myid', $myid);
                                $stmt->execute();
								...
								print '<li class="paging-nav ' . ($page == "1" ? 'disabled' : '') . '"><a ' . ($page == "1" ? '' : 'href="referees?page='.$prevpage.'"') . '><i class="fa fa-chevron-left"></i></a></li>';
					            for ($b=1;$b<=$records;$b++)
                                 {
		                        ?><li class="paging-nav <?php if ($b == $page) { echo 'active'; } ?>"><a <?php if ($b == $page) { print ' style="background-color:#2563eb; color:white;" '; } ?> href="referees?page=<?php echo "$b"; ?>"><?php echo $b." "; ?></a></li><?php
                                 }	
								 print '<li class="paging-nav ' . ($page == $records ? 'disabled' : '') . '"><a ' . ($page == $records ? '' : 'href="referees?page='.$nextpage.'"') . '><i class="fa fa-chevron-right"></i></a></li>';
								?>
						            </ul>	
					            </div>
								<?php } ?>
