...
<head>
	...
	<title>TaskBuddy - Academic Qualifications</title>
    ...
    <meta property="og:image:alt" content="Bwire " />
							...
							<div class="GridLex-col-9_sm-8_xs-12">
								...
								<div class="admin-content-wrapper">
									...
									<div class="admin-section-title">
										...
										<h2 class="admin-title-heading"><i class="fa fa-graduation-cap"></i> Academic Qualifications</h2>
										<p class="admin-title-sub">Manage your education history, degrees, and schooling credentials</p>
                                    ...
                                    $page1_int = (int)$page1;
                                    $stmt = $conn->prepare("SELECT * FROM tbl_academic_qualification WHERE member_no = :myid ORDER BY id LIMIT $page1_int,5");
                                    ...
                                    {
										?>
										<option <?php if (isset($ccountry) && $ccountry == $rowb['country_name']) { print ' selected '; } ?> value="<?php echo $rowb['country_name']; ?>"><?php echo $rowb['country_name']; ?></option>
										<?php
                                ...
                                $stmt = $conn->prepare("SELECT COUNT(*) as total FROM tbl_academic_qualification WHERE member_no = :myid");
                                $stmt->bindParam(':myid', $myid);
                                $stmt->execute();
								...
								print '<li class="paging-nav ' . ($page == "1" ? 'disabled' : '') . '"><a ' . ($page == "1" ? '' : 'href="academic?page='.$prevpage.'"') . '><i class="fa fa-chevron-left"></i></a></li>';
					            for ($b=1;$b<=$records;$b++)
                                 {
		                        ?><li class="paging-nav <?php if ($b == $page) { echo 'active'; } ?>"><a <?php if ($b == $page) { print ' style="background-color:#2563eb; color:white;" '; } ?> href="academic?page=<?php echo "$b"; ?>"><?php echo $b." "; ?></a></li><?php
                                 }	
								 print '<li class="paging-nav ' . ($page == $records ? 'disabled' : '') . '"><a ' . ($page == $records ? '' : 'href="academic?page='.$nextpage.'"') . '><i class="fa fa-chevron-right"></i></a></li>';
								?>
						            </ul>	
					            </div>
								<?php } ?>
							        ...
							        <div class="form-group"> 
								    <label>Attach Certificate <i>(Optional)</i></label>
								    <input class="form-control" accept="application/pdf,image/*" type="file" name="certificate"> 
							        </div>
							        ...
							        <div class="form-group"> 
								    <label>Academic Transcript <i>(Optional)</i></label>
								    <input class="form-control" accept="application/pdf,image/*" type="file" name="transcript"> 
							        </div>
