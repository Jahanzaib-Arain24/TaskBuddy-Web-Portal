...
<head>
	...
	<title>TaskBuddy Jobs - Other Attachments</title>
							...
							<div class="GridLex-col-9_sm-8_xs-12">
								...
								<div class="admin-content-wrapper">
									...
									<div class="admin-section-title">
										...
										<h2 class="admin-title-heading"><i class="fa fa-folder-open"></i> Documents & Attachments</h2>
										<p class="admin-title-sub">Upload copies of CNIC, portfolio certificates, or credentials</p>
                                    ...
                                    $page1_int = (int)$page1;
                                    $stmt = $conn->prepare("SELECT * FROM tbl_other_attachments WHERE member_no = :myid ORDER BY id LIMIT $page1_int,5");
								...
								try {
                                    ...
                                    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM tbl_other_attachments WHERE member_no = :myid");
                                    $stmt->bindParam(':myid', $myid);
                                    $stmt->execute();
								...
								print '<li class="paging-nav ' . ($page == "1" ? 'disabled' : '') . '"><a ' . ($page == "1" ? '' : 'href="attachments?page='.$prevpage.'"') . '><i class="fa fa-chevron-left"></i></a></li>';
					            for ($b=1;$b<=$records;$b++)
                                 {
		                        ?><li class="paging-nav <?php if ($b == $page) { echo 'active'; } ?>"><a <?php if ($b == $page) { print ' style="background-color:#2563eb; color:white;" '; } ?> href="attachments?page=<?php echo "$b"; ?>"><?php echo $b." "; ?></a></li><?php
                                 }	
								 print '<li class="paging-nav ' . ($page == $records ? 'disabled' : '') . '"><a ' . ($page == $records ? '' : 'href="attachments?page='.$nextpage.'"') . '><i class="fa fa-chevron-right"></i></a></li>';
								?>
						            </ul>	
					            </div>
								<?php } ?>
							        ...
							        <div class="form-group"> 
								    <label>Select Attachment <i>(PDF or image)</i></label>
