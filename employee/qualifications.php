...
<head>
	...
	<title>Taskbuddy - Professional Qualifications</title>
							...
							<div class="GridLex-col-9_sm-8_xs-12">
								...
								<div class="admin-content-wrapper">
									...
									<div class="admin-section-title">
										...
										<h2 class="admin-title-heading"><i class="fa fa-trophy"></i> Professional Qualifications</h2>
										<p class="admin-title-sub">Manage your professional certifications, licenses, and skill credentials</p>
                                    ...
                                    $page1_int = (int)$page1;
                                    $stmt = $conn->prepare("SELECT * FROM tbl_professional_qualification WHERE member_no = :myid ORDER BY id LIMIT $page1_int,5");
									...
									<option disabled value="">Select</option>
										...
										<option <?php if (isset($ccountry) && $ccountry == $row['country_name'] ) { print ' selected '; } ?> value="<?php echo $row['country_name']; ?>"><?php echo $row['country_name']; ?></option>
										<?php
	                                     }
                                         $stmt->execute();
                                ...
                                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                                $stmt = $conn->prepare("SELECT COUNT(*) as total FROM tbl_professional_qualification WHERE member_no = :myid");
                                $stmt->bindParam(':myid', $myid);
                                $stmt->execute();
								...
								print '<li class="paging-nav ' . ($page == "1" ? 'disabled' : '') . '"><a ' . ($page == "1" ? '' : 'href="qualifications?page='.$prevpage.'"') . '><i class="fa fa-chevron-left"></i></a></li>';
					            for ($b=1;$b<=$records;$b++)
                                 {
		                        ?><li class="paging-nav <?php if ($b == $page) { echo 'active'; } ?>"><a <?php if ($b == $page) { print ' style="background-color:#2563eb; color:white;" '; } ?> href="qualifications?page=<?php echo "$b"; ?>"><?php echo $b." "; ?></a></li><?php
                                 }	
								 print '<li class="paging-nav ' . ($page == $records ? 'disabled' : '') . '"><a ' . ($page == $records ? '' : 'href="qualifications?page='.$nextpage.'"') . '><i class="fa fa-chevron-right"></i></a></li>';
								?>
						            </ul>	
					            </div>
								<?php } ?>
				                    ...
				                    <div class="modal-header">
					                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					                 <h4 class="modal-title text-center">Add Professional Qualification</h4>
				                    </div>
